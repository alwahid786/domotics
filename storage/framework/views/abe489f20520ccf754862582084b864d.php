...
<?php $component->withAttributes([]); ?>
    ...
    <style>
        ...
        .modal-header { font-size: 20px; margin-bottom: 15px; font-weight: bold; color: #333; }
        .close-modal {
            position: absolute; top: 10px; right: 15px; font-size: 24px; color: #888; 
            cursor: pointer; border: none; background: transparent; outline: none;
        }
        .modal-body { margin-bottom: 20px; }
        .modal-body label { font-weight: 500; margin-bottom: 5px; display: block; color: #555; }
        .modal-body input, .modal-body select {
            width: 100%; padding: 8px 10px; margin-top: 5px; border: 1px solid #ddd;
            border-radius: 4px; font-size: 14px;
        }
        .modal-footer { text-align: right; }
        .modal-footer button {
            padding: 8px 16px; font-size: 14px; border-radius: 4px; border: none;
            cursor: pointer; margin-left: 8px;
        }
        .modal-footer .btn-secondary { background-color: #ccc; color: #333; }
        .modal-footer .btn-primary { background-color: #007bff; color: #fff; }
        /* Table styling */
        #sensorTable {
        ...
        .mode-active { background-color: #007bff !important; }
    </style>
    ...
    <script>
        ...
        // Data storage
        let sensorPrices = {};
        let productsData = []; // sensors array
        let totalPrice = 0;
        ...
        finalImage.addEventListener('click', function(e) {
            ...
            if(currentMode === 'floor'){
                if(!currentPolygon){
                    currentPolygon = { id: 'poly-' + Date.now(), vertices: [clickPoint], name: '' };
                } else {
        ...
        function drawFinalPolygon(polygon) {
            ...
            // Use top-left point (minimum x and y)
            const labelX = Math.min(...xs) + 5; // slight offset
            const labelY = Math.min(...ys) + 15; // slight offset
            ...
            // Delete icon: positioned close to the label (e.g. top-right inside the rectangle)
        ...
        document.getElementById('saveDot').addEventListener('click', function(){
            ...
            // Do not remove the sensor option so user may select same sensor again.
        ...
        // Generate PDF using jsPDF (existing functionality remains unchanged)
                ...
                svgImg.onload = function () {
                    ...
                    sensorTableBody.querySelectorAll('tr').forEach(row => {
                        ...
                        sensorsToSend.push({
                            ...
                            sensor: cols[2].innerText,
                            room: cols[3].innerText,
                            price: cols[4].innerText
                        });
                    });
                    ...
                    // Prepare room polygon data
                    const roomPolygons = [];
                    svgOverlay.querySelectorAll('polygon[data-room]').forEach(polygon => {
                        ...
                        const roomName = polygon.getAttribute('data-room');
                        roomPolygons.push({ roomName, points });
                    });

                    // Prepare FormData
                    const formData = new FormData();
                    formData.append('productsData', JSON.stringify(sensorsToSend));
                    formData.append('totalPrice', totalPrice);
                    formData.append('sensorCoordinates', JSON.stringify(sensorCoordinates));
                    formData.append('roomPolygons', JSON.stringify(roomPolygons));
                    formData.append('image', finalImage.blob, 'canvas-image.png');
                    formData.append('Floor name', floorNameInput.value);

                    // Console log for debug
                    console.log({
                        productsData: sensorsToSend,
                        totalPrice,
                        sensorCoordinates,
                        roomPolygons, 
                        floorNameInput
