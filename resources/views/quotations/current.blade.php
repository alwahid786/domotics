            ...
            <div class="pt-3 sm:pt-5 mb-2 p-4">
                <!-- Search and Filter -->
                            ...
                            @if ($quotation && $quotation->products->count() > 0)
                                <form action="{{ route('quotation.update') }}" method="POST" class="pt-4">
                                    ...
                                    @foreach ($productsGroupedByRoom as $roomId => $products)
                                        ...
                                        <h2 class="text-xl font-semibold text-black dark:text-white">Ambiente: {{ $room ? $room->name : 'Unknown Room' }}</h2>
                                                    ...
                                                    <div>
                                                        ...
                                                        @if (Auth::user()->hasRole('Admin') || Auth::user()->hasRole('Manager'))
                                                            <input type="number"
                                                                   ...
                                                                   style="width: 100px;"> (da listino: €{{$product->priceByRole($quotation->user)}})
                        ...
                        = €{{ $product->pivot->quantity * $product->pivot->price }}
                                    ...
                                    @if (Auth::user()->hasRole('Admin'))
                                        <div class="form-group">
                                            ...
                                            <input type="number" name="discount" id="discount"
                                                   value="{{ old('discount', $quotation->discount) }}" step="0.01"
                                                   class="form-control">
                                        </div>
                                    @endif
                                    ...
                                    <br/>
                                    <br/>
Una volta confermato il preventivo potrai, se necessario, allegare una pianta in pdf dalla sezione Preventivi.
                                    <button type="submit" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">Aggiorna preventivo</button>
                                    <a href="{{route('quotations.confirm',$quotation->id)}}" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">Conferma preventivo</a>
                                </form>
