<x-app-layout>
    ...
    function renderSensorTable() {
        ...
        const RoleId = document.getElementById('RoleId') ? document.getElementById('RoleId').value.trim() : '';
        ...
        productsData.forEach((sensor, index) => {
            ...
            if (sensor.sensorImage && sensor.sensorImage.image) {
                ...
                imageUrl = "{{ asset('storage') }}/" + sensor.sensorImage.image;
            } else if (typeof sensor.sensorImage === 'string' && sensor.sensorImage) {
                // Check if it's already a full URL (starts with http://, https://, or contains /storage/)
                if (sensor.sensorImage.startsWith('http://') || sensor.sensorImage.startsWith('https://') || sensor.sensorImage.includes('/storage/')) {
                    // Already a full URL, use it directly
                    imageUrl = sensor.sensorImage;
                } else {
                    // It's a relative path, add asset prefix
                    imageUrl = "{{ asset('storage') }}/" + sensor.sensorImage;
                }
            }
            
            // Create image HTML
            const imageHtml = imageUrl ?
                `<img src="${imageUrl}" alt="${sensor.sensor}" style="width:50px; height:50px; object-fit:contain; border-radius:4px;" onerror="this.onerror=null; this.src='https://via.placeholder.com/50?text=No+Image';">` :
                `<div style="width:50px; height:50px; display:flex; align-items:center; justify-content:center; background-color:#f0f0f0; border-radius:4px; font-size:10px; color:#666;">No Image</div>`;
            ...
            tr.innerHTML = `<td>${displayNumber}</td>
                      ...
                      <td>${sensorCode}</td>
                      <td data-sensor-type="${sensor.sensor}">${sensor.sensor}</td>
                      <td><input type="number" value="${actualPrice.toFixed(2)}" class="price-input" onchange="TotalPriceChanges(this)" /></td>
                      <td>
    ...
    function TotalPriceChanges(element) {
        ...
        const sensorType = row.querySelector('td:nth-child(7)').textContent.trim();
        
        // Update all price inputs for the same sensor type
        document.querySelectorAll('#sensorTable tbody tr').forEach(function(row) {
            const rowSensorType = row.querySelector('td:nth-child(7)').textContent.trim();
