                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="border px-4 py-2">
                                    <a href="{{ route('estimations.show', $estimation->estimation_id) }}"
                                        ...
                                        mostra</a>
                                    <a href="{{ route('estimations.edit', $estimation->estimation_id) }}" class="nline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">Modifica</a>
                                    <a href="{{ route('estimations.destroy', $estimation->estimation_id) }}" class="nline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150" onclick="event.preventDefault(); if(confirm('Are you sure you want to delete this estimation?')) { document.getElementById('delete-form-{{ $estimation->id }}').submit(); }">Cancella</a> 
                                    <form id="delete-form-{{ $estimation->estimation_id }}"
                                        action="{{ route('estimations.destroy', $estimation->estimation_id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
