<x-app-layout>
                    ...
                    <form action="{{ route('products.update', $product->id) }}" method="POST"
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="name" id="name"
                                value="{{ old('name', $product->name) }}" class="form-control border p-2 w-full"
                                required>
                            @error('name')
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="code" id="code"
                                value="{{ old('code', $product->code) }}" class="form-control border p-2 w-full"
                                required>
                            @error('name')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group mb-4">
                            ...
                            <textarea name="description" id="description" class="form-control border p-2 w-full">{{ old('description', $product->description) }}</textarea>
                            @error('description')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        {{-- <div class="form-group mb-4">
                            ...
                            <select name="rooms[]" id="rooms" class="form-control border p-2 w-full"
                                multiple="multiple" required>
                                @foreach ($rooms as $room)
                                    <option value="{{ $room->id }}"
                                        {{ in_array($room->id, old('rooms', $product->rooms->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        {{ $room->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <div class="custom-select-wrapper">
                                <div id="roomSelect" class="custom-select">
                                    @foreach ($rooms as $room)
                                        @if (in_array($room->id, old('rooms', $product->rooms->pluck('id')->toArray())))
                                            <div class="badge-room" data-id="{{ $room->id }}">
                                                {{ $room->name }}
                        ...
                        <div class="form-group mb-4">
                            ...
                            @foreach ($roles as $role)
                                <div class="mb-2">
                                    ...
                                    <input type="number" name="roles[{{ $role->id }}]"
                                        id="role_{{ $role->id }}"
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // Initialize from Blade (edit mode)
            roomSelect.querySelectorAll('.badge-room').forEach(badge => {
        ...
        `;
            ...
            function highlightOption(id) {
                const option = document.querySelector(`#roomOptions div[data-id="${id}"]`);
                if (option) {
